...
@injectable()
export class UpdateCampaignTrackingUseCase {
  constructor(
    @inject("ICampaignTrackingRepository") private readonly campaignTrackingRepository: ICampaignTrackingRepository
  ) {}

  async execute(campaignTracking: CampaignTracking): Promise<CampaignTracking | null> {
    return await this.campaignTrackingRepository.update(campaignTracking);
  }
}
